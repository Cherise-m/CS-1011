...
public class District {
    ...
    private int totalTimeClosed, previousTime;
    private int total;
    ...
    public int getNumberOfSpotsRemaining() {
        ...
        for (int g = 0; g < numLots; g++) {
            lots[g].getNumberOfSpotsRemaining();
            spotsOpen += lots[g].getNumberOfSpotsRemaining();
        }
        return spotsOpen;
    }
    ...
    public void markVehicleEntry(int lotNumber, int timestamp) {
        ...
        if (previousTime <= timestamp) {
            lots[lotNumber].markVehicleEntry(timeClosed);
    ...
    public void markVehicleExit(int lotNumber, int timestamp) {
        ...
        alreadyClosed = isClosed();
        if (previousTime <= timestamp) {
            lots[lotNumber].markVehicleExit(timeOpened);
